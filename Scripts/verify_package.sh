...
if /usr/libexec/PlistBuddy -c 'Print :Filter:Bundles' "$filter" | grep -E 'com\.apple\.|com\.apple\.UIKit' >/dev/null; then
  echo "Apple or UIKit bundle found in Satella allowlist" >&2
  exit 1
fi
